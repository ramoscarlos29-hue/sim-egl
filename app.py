...
if not st.session_state.examen_iniciado:
    ...
    st.info("Bienvenido, Felino. Completa tu registro para iniciar.")
    ...
    # --- RESULTADOS FINALES ---
        ...
        if not st.session_state.registro_intentado:
            try:
                ...
                st.warning("⚠️ No se pudo sincronizar el registro. Descarga tu PDF.")
        ...
        def c_txt(t): return str(t).encode('latin-1', 'replace').decode('latin-1')
        
        pdf.cell(190, 8, c_txt(f"Estudiante: {st.session_state.nombre}"), 0, 1)
        pdf.cell(190, 8, c_txt(f"Puntaje: {st.session_state.puntaje}/{len(df)}"), 0, 1)
        pdf.ln(10)
        
        for _, r in df_res.iterrows():
            pdf.cell(190, 8, c_txt(f"{r['Subarea']}: {r['Aciertos']} - {r['Semaforo']}"), 1, 1)

        pdf_bytes = pdf.output()
        st.download_button("📥 Descargar Reporte PDF", pdf_bytes, f"Resultado_{st.session_state.nombre}.pdf", "application/pdf")
